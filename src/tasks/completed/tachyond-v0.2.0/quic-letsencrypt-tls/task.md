...
- [x] Terraform で `QUIC_TLS_CERT_SECRET` 環境変数を追加
- [x] Let's Encrypt 証明書の取得・Secrets Manager 格納
- [x] 本番デプロイ・動作確認（`--quic-insecure` なしで接続成功）
...
### Phase 2: 本番デプロイ (2026-02-13)

1. certbot で `quic.n1.tachy.one` の Let's Encrypt 証明書を DNS-01 チャレンジで取得
2. AWS Secrets Manager に `tachyon/quic-tls` として cert_pem + key_pem を格納
3. Terraform apply で ECS タスク定義に `QUIC_TLS_CERT_SECRET` 環境変数を追加
4. ECS 再デプロイ → Secrets Manager から証明書をロードしていることをログで確認
5. tachyond から `--quic-insecure` なしで QUIC 接続成功を確認
