...
### フェーズ1: concurrency追加 ✅
- [x] 対象12ワークフローにconcurrency設定を追加
- [x] `rust.yaml` と `tidb-migration-check.yaml` は既に設定済みであることを確認

### フェーズ2: ドラフトPRスキップ ✅
- [x] 各ワークフローのトリガーに `ready_for_review` typeを追加（全14ワークフロー）
- [x] 主要ジョブにドラフトPRスキップ条件を追加（`github.event.pull_request.draft != true`）
- [x] pushイベントでは引き続き実行される（`draft`がundefined→`!= true`で通過）

### フェーズ3: yarn.lock paths フィルタ見直し ✅
- [x] 案2を採用: 全フロントCI・Claude Code Reviewの`on.paths`と`paths-filter`から`yarn.lock`を除去
- [x] 既存の paths-filter（共有パッケージ `packages/react/**` 等を含む）はそのまま維持
- [x] yarn.lock のみの変更ではワークフローがトリガーされない
- [x] yarn.lock + アプリソース or 共有パッケージ変更ではアプリソース側のパスマッチで実行される

### フェーズ4: Chromatic main push 失敗の調査 📝
- [ ] `chromatic-tachyon` の失敗原因を調査
- [ ] 必要に応じて修正タスクを起票

### 追加施策: Rust CI mainブランチpush除去 ✅
- [x] `rust.yaml`から`push`トリガーを完全除去（PRで検証済みのため不要）
- [x] push時のみ必要だった`actions/checkout`ステップも除去
